...
//! cadence from a fresh [`orchestrail_engine::state::Snapshot`] (queue + task descriptors),
...
fn handle_key(
    ...
    match k.code {
        ...
        KeyCode::Char('p') if k.modifiers.is_empty() => {
            run_pause(app, &cfg.work_dir, last_status_reload)
        }
        KeyCode::Char('u') if k.modifiers.is_empty() => {
            run_resume(app, &cfg.work_dir, last_status_reload)
        }
